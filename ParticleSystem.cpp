...
void ParticleSystem::addParticle(float mass, const sf::Vector2f& position, const sf::Vector2f& velocity, const sf::Color& color) {
...
void ParticleSystem::applyInteractiveForces(float strength) {
    for (size_t i = 0; i < m_particles.size(); ++i) {
        for (size_t j = i + 1; j < m_particles.size(); ++j) {
            ...
            // Se strength > 0: repulsão, se strength < 0: atração
...
void ParticleSystem::generateRandomParticles(int count, float minMass, float maxMass) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        addParticle(mass, position, velocity, color);
    }
}
